    ...
    <div id="sw-${key}-main">
        ...
        <div class="tbar no-capture">
          <div class="tb-l">
            <div class="srch">
              ...
              <input placeholder="Search…" class="roster-search" data-search-key="${key}">
            </div>
        ...
        <div class="twrap" id="sw-${key}-tbl">
          <table id="tbl-${key}">
            <thead><tr>
              ...
              <th class="no-capture">Actions</th>
            </tr></thead>
      ...
      <div class="tab-pane" id="sw-${key}-growth">
        ...
        <div class="dnote no-capture" style="margin-bottom:10px">
          ...
          Growth recalculates instantly. Default sort: <strong>highest recent growth</strong>.
        </div>
...
function renderTable(key) {
  ...
  tbody.innerHTML = (window.DB[key] || []).map((c, i) => {
    ...
    return `<tr class="${rowCls(c.status)}${hid ? ' row-hidden' : ''}"
      data-search="${(c.name + ' ' + (c.status || '') + ' ' + (c.profession || '')).toLowerCase()}"
      data-gender="${gender}"
      data-id="${c.id}">
...
function filterGender(key, val) {
  ...
  document.querySelectorAll(`#tb-${key} tr`).forEach(tr => {
    const g = (tr.dataset.gender || '').toUpperCase().trim();
    tr.style.display = !v || g === v ? '' : 'none';
  });
}
...
function buildGrowthHTML(key) {
  ...
  const rows = data.map((c, idx) => {
    ...
    const g2col = g2.rateRaw !== null && g2.rateRaw >= 50 ? 'var(--gtbl-hi)' :
                  g2.rateRaw !== null && g2.rateRaw < 0   ? 'var(--gtbl-neg)' : 'var(--gtbl-txt)';
    ...
    return `<tr draggable="true" data-id="${c.id}" data-key="${key}"
      ...
      <td style="text-align:center;font-weight:800;color:var(--mut)">${idx + 1}</td>
      <td style="font-weight:700;text-align:left">${eC(c.name, 'name')}</td>
      ...
      <td style="text-align:center;color:${g2col}">${g2.rate}</td>
    </tr>`;
  }).join('');
  ...
  return `<div class="growth-sort-bar no-capture">
    ...
    <select onchange="setGrowthSort('${key}', this.value)">
      <option value="growth-desc"    ${sortVal === 'growth-desc'    ? 'selected' : ''}>📈 Highest Growth (Recent)</option>
...
function filterRankings() {
  const status = (document.getElementById('rank-status-filter')?.value || '').toLowerCase();
  ...
  document.querySelectorAll('#rank-tbody tr').forEach(tr => {
    const ok = (!status || (tr.dataset.status || '').includes(status))
            && (!show   || tr.dataset.show === show)
            && (!gender || (tr.dataset.gender || '').toUpperCase() === gender)
...
document.addEventListener('DOMContentLoaded', () => {
  const _dFilter = debounce((key, val) => filterTbl(key, val), 280);
  document.body.addEventListener('input', e => {
    const inp = e.target;
    if (inp.classList.contains('roster-search')) {
      _dFilter(inp.dataset.searchKey, inp.value);
    }
  });
