...
function filterTable(key) {
  ...
  const total = document.querySelectorAll(`#tb-${key} tr`).length;
...
function buildGrowthHTML(key) {
  ...
  <div class="gtbl-wrap" id="gtbl-inner-${key}">
    <div class="gtbl-title" style="border-left:3px solid ${s?.color || 'var(--acc)'};">${s?.emoji || ''} ${s?.label || key.toUpperCase()} — Instagram Follower Growth</div>
    <table>
      <thead><tr>
        <th style="width:50px;text-align:center">#</th>
        <th style="text-align:center;min-width:160px">Contestant</th>
