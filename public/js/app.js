...
function buildGrowthHTML(key) {
  ...
  const rows = data.map((c,idx) => {
    ...
    const g2Color = g2.rateRaw!==null && g2.rateRaw>=50 ? 'var(--gtbl-hi)' : g2.rateRaw!==null && g2.rateRaw<0 ? 'var(--gtbl-neg)' : 'var(--gtbl-txt)';
    ...
    return `<tr draggable="true" data-id="${c.id}" data-key="${key}"
      ...
      <td style="text-align:center;font-weight:800">${rank}</td>
      <td style="font-weight:800;text-align:left">${eC(c.name,'name')}</td>
